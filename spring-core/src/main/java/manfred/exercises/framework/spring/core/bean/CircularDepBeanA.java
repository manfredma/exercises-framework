package manfred.exercises.framework.spring.core.bean;

/**
 * Spring Bean 循环依赖演示类 A，持有对 CircularDepBeanB 的引用并在构造和 setter 中打印调用栈。
 * 演示 Spring 三级缓存如何解决 A、B 互相依赖的循环注入问题。
 */
public class CircularDepBeanA {

    private CircularDepBeanB b;

    public CircularDepBeanA() {
        new RuntimeException("调用A构造方法").printStackTrace(System.out);
    }

    public CircularDepBeanB getB() {
        return b;
    }

    public void setB(CircularDepBeanB b) {
        this.b = b;
        new RuntimeException("调用A的方法setB").printStackTrace(System.out);
    }

    @Override
    public String toString() {
        return "CircularDepBeanA{b=" + System.identityHashCode(b) + '}';
    }
}
